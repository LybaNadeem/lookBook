...
class EditPhotographer extends StatefulWidget {
  var productId;
  var photographerId;


  EditPhotographer({Key? key, required this.productId, required this.photographerId}) : super(key: key);
  @override
  _EditPhotographerState createState() => _EditPhotographerState();
}

class _EditPhotographerState extends State<EditPhotographer> {
  // Controllers
  ...
  void initState() {
    super.initState();
    fetchProductAndPhotographerData();
  }

  Future<void> fetchProductAndPhotographerData() async {
    try {
      // Fetch product data
      ...
      if (productSnapshot.exists) {
        ...
        if (productData != null) {
          setState(() {
            ...
            _imageFile = _imageUrl != null ? File(_imageUrl!) : null;
            photographerNameController.text = productData['photographerName'] ?? '';
            socialLinkController.text = productData['socialLink'] ?? '';
            phoneController.text = productData['phone'] ?? '';
            emailController.text = productData['email'] ?? '';
      ...
      if (photographerSnapshot.exists) {
        Map<String, dynamic>? photographerData = photographerSnapshot.data() as Map<String, dynamic>?;
        if (photographerData != null) {
          setState(() {
            photographerNameController.text = photographerData['photographerName'] ?? '';
            socialLinkController.text = photographerData['socialLink'] ?? '';
            phoneController.text = photographerData['phone'] ?? '';
            emailController.text = photographerData['email'] ?? '';
          });
        }
      }
    } catch (e) {
